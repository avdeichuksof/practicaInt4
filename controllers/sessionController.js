...
class SessionController {
    ...
    updateRoleController = async (req, res) => {
        ...
        const userPath = `src/files/users/${userId}`
        const filesFound = fs.readdirSync(userPath)

        // verificamos que existan los  documentos
        const dniFound = filesFound.some((file) => file.startsWith('dni-'))
        const comprobanteDomicilioFound = filesFound.some((file) => file.startsWith('comprobanteDomicilio-'))
        const estadoCuentaFound = filesFound.some((file) => file.startsWith('estadoCuenta-'))

        if (dniFound && comprobanteDomicilioFound && estadoCuentaFound) {
            ...
            res.status(500).send({message: "No se pudo cambiar el rol"})
        }
    }
